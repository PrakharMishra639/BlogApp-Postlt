...
const Comment = ({comment ,logginedUserId, affectedComment, setAffectedComment,addComment,parentId=null,
    updateComment,deleteComment,replies=[]}) => {
  ...
  return (
    ...
    id={`comment-${comment?._id}`}>
       <img src={comment?.user?.avatar ? stables.UPLOAD_FOLDER_BASE_URL + comment.user.avatar : images.userImage} alt="userProfile" className="w-9 h-9 object-cover rounded-full " />
      ...
      <div className=" flex items-center gap-x-3 text-dark-light font-roboto text-sm mt-3 mb-3">
        {
            isUserLoggined && (<button className="flex itms-center space-x-2" onClick={() =>setAffectedComment({type:'replying',_id:comment._id})}>
                <FiMessageSquare className="w-4 h-auto" />
                    <span>Reply</span>
                </button>)
        }
        {commentBelongsToUser && (<><button className="flex itms-center space-x-2" 
         onClick={() =>setAffectedComment({type:'editing',_id:comment._id})}>
        <MdOutlineEdit className="w-4 h-auto" />
            <span>Edit</span>
        </button>
        <button className="flex itms-center space-x-2" onClick={() => deleteComment(comment._id)}>
        <RiDeleteBin6Line  className="w-4 h-auto" />
            <span>Delete</span>
        </button></>)}
      </div>
      {isReplying && <CommentForm btnLabel="Reply" formSubmitHandler={(value)=>addComment(value,repliedCommentId ,replyOnUserId)}
        ...
        {replies.length > 0 && (
            <div>
                {replies.map((reply) => (
                    <Comment key={reply._id} addComment={addComment} 
