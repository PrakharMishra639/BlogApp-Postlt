...
const Comments = () => {
  ...
  const {mutate :mutateUpdateCommentCheck, isLoading :isLoadingCommentCheck} = useMutation({
  ...
  return (
    <DataTable pageTitle="Manage Comments"
    ...
    {commentsData?.data.map((comment) =>(
       ...
       {comment?.replyOnUser !== null && (
            <p className="text-gray-900 whitespace-no-wrap">
              In reply to <Link to={`/blog/${comment?.post?.slug}/#comment-${comment?._id}`}
       ...
       <td className="px-5 py-5 text-sm bg-white border-b border-gray-200 space-x-5">
         <button
           ...
           onClick={() => {
          mutateUpdateCommentCheck({token: userState.userInfo.token, check:comment?.check ? false : true,
