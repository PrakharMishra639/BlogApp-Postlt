...
const SuggestedPosts = ({ className, header, posts = [], tags }) => {
  return (
    <div className={`w-full shadow-[rgba(7,_65,_210,_0.1)_0px_9px_30px] rounded-lg p-4 ${className}`}>
      ...
      {tags.length===0 ?
       (<p className="text-slate-500 test-xs mt-2">
        There is no tags for this posts
       </p>)
      :(
      <div className="flex flex-wrap gap-x-2 gap-y-2 mt-4">
        {tags.map((item,index) => (
          <Link key={index} to="/" className="inline-block rounded-md px-3 py-1.5 bg-primary font-roboto  md:text-sm text-xs text-white">{item}</Link>
        ))}
      </div>)}
    </div>
  );
